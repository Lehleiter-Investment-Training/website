// Glossar-Daten für den Optionshandel.
// Werden serverseitig von src/bonus/glossar.njk gerendert (crawl- und zitierbar
// für Such- und KI-Engines) sowie als DefinedTermSet-Schema ausgegeben.
// Die clientseitige Suche (assets/js/glossar.js) filtert nur das bereits
// gerenderte HTML – die Daten leben ausschließlich hier.
...
// Alphabetisch sortieren (locale-aware), damit die Buchstaben-Gruppierung stimmt.
const sorted = [...terms].sort((a, b) => a.term.localeCompare(b.term, "de"));

// Nach Anfangsbuchstaben gruppieren – für die serverseitige Sektions-Ausgabe.
const byLetterMap = {};
const byLetter = [];
for (const item of sorted) {
  const letter = item.term[0].toUpperCase();
  if (!byLetterMap[letter]) {
    byLetterMap[letter] = { letter, items: [] };
    byLetter.push(byLetterMap[letter]);
  }
  byLetterMap[letter].items.push(item);
}
byLetter.sort((a, b) => a.letter.localeCompare(b.letter, "de"));

// Alphabet-Navigation: alle A–Z, markiert welche Buchstaben belegt sind.
const usedLetters = new Set(byLetter.map((g) => g.letter));
